...
data class Notification
    ...
    private constructor(
        ...
        val creationInstant: Instant = Instant.now(), // instant in which the notification was created (internal)
        val media: NotificationMedia?,
        val alertLevel: AlertLevel,
        val refUri: URI?,
        val contextData: Any?,
        val issuanceInstant: Instant = Instant.now(), // instant in which the (potentially external) event occurred
        ...
        data class Builder(
            ...
            fun media(media: NotificationMedia) =
                apply {
                    this.media = media
                }

            fun unique(isUnique: Boolean) =
                apply {
                    this.isUnique = isUnique
                }

            fun title(title: I18nText) =
                apply {
                    this.title = title
                }

            fun message(message: I18nText) =
                apply {
            ...
            fun alertLeveL(alertLevel: AlertLevel) =
                apply {
                    this.alertLevel = alertLevel
                }

            fun instantOfIssue(instantOfIssue: Instant) =
                apply {
                    this.instantOfIssue = instantOfIssue
                }

            fun needsExplicitDismiss(needsExplicitDismiss: Boolean) =
                apply {
                    this.needsExplicitDismiss = needsExplicitDismiss
                }

            fun ignoreNotification(ignoreNotification: Boolean) =
                apply {
                    this.ignoreNotification = ignoreNotification
                }

            fun build(): Notification? =
                if (!this.ignoreNotification && this.title != null && this.message != null) {
                    Notification(
                        this.targetId,
                        this.targetType,
                        this.objectId,
